 ...
 *
 *   $Id: admin_groups.php,v 1.7 2003/08/30 15:05:44 acydburn Exp $
 *
 *
 ***************************************************************************/
...
if ( isset($HTTP_POST_VARS['edit']) || isset($HTTP_GET_VARS['edit']) || isset($HTTP_POST_VARS['new']) )
{
	...
	$template->set_filenames(array(
		'body' => 'service/group_edit_body.tpl')
	);
...
else if ( isset($HTTP_POST_VARS['group_update']) )
{
	...
	if ( isset($HTTP_POST_VARS['group_delete']) )
	{
		...
		if (intval($row['auth_mod']) == 1)
		{
			...
			for ($i = 0; $i < count($rows); $i++)
			{
				...
				if ($db->sql_numrows($result) == 0)
				{
					...
					log_action('update[moderator-permissions]', 0, $userdata['user_id'], $userdata['username']);
					// Log actions MOD End		
				}
			}
		}
		...
		// Log actions MOD Start
		log_action('delete[group]', $group_id, $userdata['user_id'], $userdata['username']);
		// Log actions MOD End
		...
		if( $mode == "editgroup" )
		{
			...
			if ( $group_info['group_moderator'] != $group_moderator )
			{
				if ( $delete_old_moderator )
				{
					...
					// Log actions MOD Start
					log_action('delete[old-moderator]', $group_id, $userdata['user_id'], $userdata['username']);
					// Log actions MOD End
			...
			// Log actions MOD Start
			log_action('update[group]', $group_id, $userdata['user_id'], $userdata['username']);
			// Log actions MOD End		
...
else
{
	...
	$template->set_filenames(array(
		'body' => 'service/group_select_body.tpl')
	);
