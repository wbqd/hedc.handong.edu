...
{
	global $lang, $phpEx, $template, $db, $board_config;
	global $qbar_maps;
	...
	$template->set_filenames(array(
		'body' => 'service/qbar_admin_body.tpl')
	);
	...
	$template->assign_vars(array(
		...
		"U_HELP" => append_sid("http://www.ontoin.com/korean/viewtopic.$phpEx?popup=yes&today=no"),
		...
		if ( empty($panel_id) || ($panel_id == $i) )
		{
			...
			if ( !empty($panel_id) || isset($open_ids[$i]) )
			{
				...
				while ( list($field_name, $field) = @each($qbar['fields']) )
				{
					...
					$field_tree_type	= substr($field['tree_id'], 0, 1);
					$field_tree_id	= intval(substr($field['tree_id'], 1));
					...
					$template->assign_block_vars('qbar.field', array(
						...
						'TITLE'				=> ((qbar_get_tree_title($field['tree_id']))? qbar_get_tree_title($field['tree_id']) : qbar_get_value($field['shortcut'])),
						'COLSPAN'			=> ((empty($field['icon']))? 2 : 1),
						...
						'ICON2'				=> qbar_get_icon(qbar_image($field['icon2'], $qbar['style'], $qbar['sub_template'])),
						...
						'FIELD_TREE_TYPE'	=> $field_tree_type,
					...
					if (!empty($field['tree_id']) && ($qbar_name != 'default_tree') && $field_tree_type == POST_FORUM_URL)
					{
						$template->assign_block_vars('qbar.field.switch_single', array());
					}
					if (!empty($field['tree_id']) && ($qbar_name != 'default_tree') && $field_tree_type == POST_CAT_URL)
					{
						$template->assign_block_vars('qbar.field.switch_multi', array());
					}
					if (!empty($field['icon']))
					{
						$template->assign_block_vars('qbar.field.switch_icon', array());
					}
...
if (($mode == 'up') || ($mode == 'dw'))
{
	...
	log_action('move[subMenu]', 0, $userdata['user_id'], $userdata['username']);
	// Log actions MOD End
}
//
// deleting
...
if ($mode == 'delete')
{
	...
	else if ($submit)
	{
		...
		// Log actions MOD Start
		log_action('delete[subMenu]', 0, $userdata['user_id'], $userdata['username']);
		// Log actions MOD End
	}
	else
	{
		// template
		$template->set_filenames(array(
			'body' => 'service/qbar_confirm_body.tpl')
		);
...
if (($mode == 'edit') && (!empty($field_id) || $create_field))
{
	...
	if (!empty($field_id))
	{
		...
		$field_icon2		= $qbar_maps[$qname]['fields'][$fname]['icon2'];
	...
	if (isset($HTTP_POST_VARS['icon2']))		$field_icon2		= trim(stripslashes($HTTP_POST_VARS['icon2']));
	...
	if ($import)
	{
		...
		else if ($submit)
		{
			...
			if (!empty($field_ids))
			{
				...
				for ($i=0; (($i==0) && empty($sav_mode)) || (($i < count($field_ids)) && !empty($sav_mode)); $i++)
				{
					...
					$field_icon2		= $qbar_maps[$from_qname]['fields'][$from_fname]['icon2'];
		...
		else
		{
			...
			$s_hidden_fields .= '<input type="hidden" name="icon2" value="' . $field_icon2 . '" />';
	...
	if ($mode == 'edit')
	{
		...
		else if ($submit)
		{
			...
			// Log actions MOD Start
			log_action('edit[subMenu-field]', 0, $userdata['user_id'], $userdata['username']);
			// Log actions MOD End
		}
		else
		{
			// template
			$template->set_filenames(array(
				'body' => 'service/qbar_field_body.tpl')
			);
			...
			$template->assign_vars(array(
				...
				'L_ICON2'				=> $lang['Qbar_icon2'],
				'L_ICON2_EXPLAIN'		=> $lang['Qbar_icon2_explain'],
				'L_USE_VALUE'			=> $lang['Qbar_use_value'],
			...
			$icon2_tr		= qbar_get_icon(qbar_image($field_icon2, $qbar_maps[$qname]['style'], $qbar_maps[$qname]['sub_template']));
...
if (($mode == 'edit') && empty($field_id) && !$create_field)
{
	...
	else if ($submit)
	{
		...
		// Log actions MOD Start
		log_action('edit[subMenu-panel]', 0, $userdata['user_id'], $userdata['username']);
		// Log actions MOD End
	...
	else
	{
		// template
		$template->set_filenames(array(
			'body' => 'service/qbar_panel_body.tpl')
		);
