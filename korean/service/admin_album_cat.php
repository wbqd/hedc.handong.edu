...
if( !isset($HTTP_POST_VARS['mode']) )
{
	if( !isset($HTTP_GET_VARS['action']) )
	{
		$template->set_filenames(array(
			'body' => 'service/album_cat_body.tpl')
		);
	...
	else
	{
		if( $HTTP_GET_VARS['action'] == 'edit' )
		{
			...
			$template->set_filenames(array(
				'body' => 'service/album_cat_new_body.tpl')
			);
		...
		else if( $HTTP_GET_VARS['action'] == 'delete' )
		{
			...
			$template->set_filenames(array(
				'body' => 'service/album_cat_delete_body.tpl')
			);
...
else
{
	...
	else if( $HTTP_POST_VARS['mode'] == 'delete' )
	{
		...
		if( $target == 0 ) // Delete All
		{
			...
			log_action('delete[album-cat]', $cat_id, $userdata['user_id'], $userdata['username']);
			// Log actions MOD End
