	...
	switch($perform)
	{
		case 'backup':
			...
			if ($error)
			{
				...
				$template->set_filenames(array(
					"body" => "service/admin_message_body.tpl")
				);
			...
			$tables = array(
				...
				//attach_mod - 8
				'attachments_config','forbidden_extensions','extension_groups','extensions','attachments_desc','attachments','quota_limits','attach_quota',

				//album - 5

				'album','album_rate','album_comment','album_cat','album_config',
			...
			if( !isset($HTTP_POST_VARS['backupstart']) && !isset($HTTP_GET_VARS['backupstart']))
			{
				...
				$template->set_filenames(array(
					"body" => "service/db_utils_backup_body.tpl")
				);	
		...
		case 'restore':
			if(!isset($HTTP_POST_VARS['restore_start']))
			{
				...
				$template->set_filenames(array(
					"body" => "service/db_utils_restore_body.tpl")
				);
