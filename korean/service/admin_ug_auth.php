 ...
 *
 *   $Id: admin_ug_auth.php,v 1.6 2003/08/30 15:05:44 acydburn Exp $
 *
 *
 ***************************************************************************/
...
if ( isset($HTTP_POST_VARS['submit']) && ( ( $mode == 'user' && $user_id ) || ( $mode == 'group' && $group_id ) ) )
{
	...
	if ( $mode == 'user' && $HTTP_POST_VARS['userlevel'] == 'admin' && $user_level != ADMIN )
	{
		...
		if ( $userdata['user_id'] != $user_id )
		{
			...
			log_action('update[user-level]', $user_id, $userdata['user_id'], $userdata['username']);
			// Log actions MOD End
			...
			// Log actions MOD Start
			log_action('update[auth]', $group_id, $userdata['user_id'], $userdata['username']);
			// Log actions MOD End
			...
			// Log actions MOD Start
			log_action('update[auth-access]', $group_id, $userdata['user_id'], $userdata['username']);
			// Log actions MOD End
	...
	else
	{
		if ( $mode == 'user' && $HTTP_POST_VARS['userlevel'] == 'user' && $user_level == ADMIN )
		{
			...
			if ( $userdata['user_id'] != $user_id )
			{
				...
				// Log actions MOD Start
				log_action('update[user-level-auth-access]', $user_id, $userdata['user_id'], $userdata['username']);
				// Log actions MOD End
...
//-- fin mod : categories hierarchy ----------------------------------------------------------------
			...
			while( list($forum_id, $action) = @each($forum_auth_action) )
			{
				...
				else
				{
					...
					// Log actions MOD Start
					log_action('update[private-forum]', $group_id, $userdata['user_id'], $userdata['username']);
					// Log actions MOD End
				}
			}
			...
			if ( $delete_sql != '' )
			{
				...
				// Log actions MOD Start
				log_action('delete[permission]', $group_id, $userdata['user_id'], $userdata['username']);
				// Log actions MOD End
			}
		...
		// Log actions MOD Start
		log_action('update[user-level]', 0, $userdata['user_id'], $userdata['username']);
		// Log actions MOD End
...
else
{
	...
	$template->set_filenames(array(
		'body' => ( $mode == 'user' ) ? 'service/user_select_body.tpl' : 'service/auth_select_body.tpl')
	);
