	...
	$sql = "SELECT * 
		...
		for ($i = 0; $i < $numrows; $i++) 
		{
         ...
         $template->assign_block_vars('record_row', array( 
            ...
            'USER_ID' => $user_id, 
            'USERNAME' => '<a href="' . $temp_url . '" target=_new>' . $username . '</a>', 
			'USER_IP' => $user_ip,
			'U_WHOIS_IP' => 'http://network-tools.com/default.asp?host=' . $user_ip . '&targetnic=auto', 
...
if ( $log_list_sql != '' )
{
	if ( $delete )
	{
		...
		else
		{
			...
			log_action('delete[log]', 0, $userdata['user_id'], $userdata['username']);
			// Log actions MOD End
	...
	$sql = "SELECT count(*) AS total
	   ...
	   { 
	      ...
	      $pagination = generate_pagination("admin_logs.$phpEx?mode=$mode&amp;order=$sort_order", $total_records, $board_config['topics_per_page'], $start, FALSE). '&nbsp;'; 
	   } 
