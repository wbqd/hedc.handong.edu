 ...
 *
 *   $Id: admin_attachments.php,v 1.48 2003/08/30 15:47:39 acydburn Exp $
 *
 ***************************************************************************/
...
while ($row = $db->sql_fetchrow($result))
{
	...
	if ( $submit && ( $mode == 'manage' || $mode == 'cats') )
	{
		...
		log_action('update[attach-config]', 0, $userdata['user_id'], $userdata['username']);
		// Log actions MOD End
...
if ($mode == 'manage')
{
	...
	$template->set_filenames(array(
		'body' => 'service/attach_manage_body.tpl')
	);
...
if ($mode == 'shadow')
{
	...
	//
	$template->set_filenames(array(
		'body' => 'service/attach_shadow.tpl')
	);
...
{
	$template->set_filenames(array(
		'body' => 'service/attach_cat_body.tpl')
	);
...
if ($mode == 'quota')
{
	...
	$template->set_filenames(array(
		'body' => 'service/attach_quota_body.tpl')
	);
