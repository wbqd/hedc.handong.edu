...
if ($mode == 'del')
{
	...
	else if ($confirm)
	{
		...
		log_action('delete[icon]', 0, $userdata['user_id'], $userdata['username']);
		// Log actions MOD End
	...
	if ($mode == 'del')
	{
		if (isset($map_icon[$icon]))
		{
			...
			$template->set_filenames(array(
				'body' => 'service/admin_icons_delete_body.tpl')
			);
...
if ( ($mode == 'up') || ($mode == 'dw') )
{
	...
	// Log actions MOD Start
	log_action('move[icon]', 0, $userdata['user_id'], $userdata['username']);
	// Log actions MOD End
...
if ($mode == 'edit')
{
	...
	else if ($submit)
	{
		...
		// Log actions MOD Start
		log_action('edit[icon]', 0, $userdata['user_id'], $userdata['username']);
		// Log actions MOD End
	...
	if ($mode == 'edit')
	{
		// template
		$template->set_filenames(array(
			'body' => 'service/admin_icons_edit_body.tpl')
		);
...
if ($mode == '')
{
	...
	// template
	$template->set_filenames(array(
		'body' => 'service/admin_icons_body.tpl')
	);
