...
if( !empty($setmodules) )
{
	...
	$module['Styles']['skin_manage'] = "$file";
...
if ($cancel)
{
	redirect('service/' . append_sid("admin_styles.$phpEx", true));
}
...
switch( $mode )
{
		...
		else
		{
			...
			if( $dir = @opendir($phpbb_root_path. "templates/") )
			{
				...
				$template->set_filenames(array(
					"body" => "service/styles_addnew_body.tpl")
				);
	...
	case "edit":
		...
		if( $submit )
		{
			...
			if($mode == "edit")
			{
				...
				log_action('update[themes]', $style_id, $userdata['user_id'], $userdata['username']);
				// Log actions MOD End				
				...
				// Log actions MOD Start
				log_action('update[themes-name]', $style_id, $userdata['user_id'], $userdata['username']);
				// Log actions MOD End									
		...
		else
		{
			...
			$template->set_filenames(array(
				"body" => "service/styles_edit_body.tpl")
			);
	...
	case "export";
		...
		{
			$template->set_filenames(array(
				"body" => "service/styles_exporter.tpl")
			);
	...
	case "delete":
		...
		else
		{
			...
			// Log actions MOD Start
			log_action('delete[style]', $style_id, $userdata['user_id'], $userdata['username']);
			// Log actions MOD End
			...
			// Log actions MOD Start
			log_action('update[user-style-information]', $style_id, $userdata['user_id'], $userdata['username']);
			// Log actions MOD End					
	...
	default:
		...
		$template->set_filenames(array(
			"body" => "service/styles_list_body.tpl")
		);
