 ...
 *
 *   $Id: admin_extensions.php,v 1.24 2003/08/30 15:47:39 acydburn Exp $
 *
 ***************************************************************************/
...
if ($mode == 'extensions')
{
	...
	$template->set_filenames(array(
		'body' => 'service/attach_extensions.tpl')
	);
...
if ($mode == 'groups')
{
	...
	//
	$template->set_filenames(array(
		'body' => 'service/attach_extension_groups.tpl')
	);
...
if ($submit && $mode == 'forbidden')
{
	...
	if( $extension_id_sql != '' )
	{
		...
		log_action('delete[forbidden]', 0, $userdata['user_id'], $userdata['username']);
		// Log actions MOD End
	}
...
{
	$template->set_filenames(array(
		'body' => 'service/attach_forbidden_extensions.tpl')
	);
...
if (($e_mode == 'perm') && ($group != -1))
{
	...
	$template->set_filenames(array(
		'perm_box' => 'service/extension_groups_permissions.tpl')
	);
	...
	if (count($empty_perm_forums) > 0)
	{
		...
		$template->assign_vars(array(
			'ERROR_MESSAGE' => "<font color=red>".$lang['Note_admin_empty_group_permissions'] .$error_msg."</font>")
		);
