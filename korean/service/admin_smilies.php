...
if( isset($HTTP_GET_VARS['import_pack']) || isset($HTTP_POST_VARS['import_pack']) )
{
	...
	if ( !empty($smile_pak) )
	{
		...
		if( !empty($clear_current)  )
		{
			...
			log_action('delete[curren-smiles]', 0, $userdata['user_id'], $userdata['username']);
			// Log actions MOD End
		}
		else
		{
	...
	else
	{
		...
		$template->set_filenames(array(
			"body" => "service/smile_import_body.tpl")
		);
...
else if( isset($HTTP_POST_VARS['add']) || isset($HTTP_GET_VARS['add']) )
{
	...
	$template->set_filenames(array(
		"body" => "service/smile_edit_body.tpl")
	);
...
else if ( $mode != "" )
{
	switch( $mode )
	{
		case 'delete':
			...
			// Log actions MOD Start
			log_action('delete[smily]', $smiley_id, $userdata['user_id'], $userdata['username']);
			// Log actions MOD End
		...
		case "save":
			...
			// Log actions MOD Start
			log_action('update[smilies-info]', 0, $userdata['user_id'], $userdata['username']);
			// Log actions MOD End
...
else
{
	...
	$template->set_filenames(array(
		"body" => "service/smile_list_body.tpl")
	);
