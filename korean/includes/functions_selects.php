...
function user_select($select_name,$lang_array,$user_data)
{
	global $lang;

    $user_select = '<select name="' . $select_name . '">';
	   while(list($value, $key) = @each($lang_array)) {
	         $selected = ($value == $user_data) ? ' selected="selected"' : '';
	         $user_select .= '<option value="' . $value . '"' . $selected . '>' . $key . '</option>';
       }
       $user_select .= '</select>';
	   return $user_select;
}
