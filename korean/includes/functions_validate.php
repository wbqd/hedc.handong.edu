...
//
// Check to see if Jumin is already present in the DB
//
function validate_jumin($jumin1,$jumin2)
{
	global $db, $lang;
	...
	if ($jumin1 != '' && $jumin2 != '')
	{
			...
			$sql = "SELECT user_jumin1, user_jumin2  
				FROM " . USERS_TABLE . "
				WHERE user_jumin1 = '" . $jumin1 . "' and user_jumin2 = '" . $jumin2 . "'";
			if (!($result = $db->sql_query($sql)))
			{
				message_die(GENERAL_ERROR, "Couldn't obtain user jumin information.", "", __LINE__, __FILE__, $sql);
			}
		
			if ($row = $db->sql_fetchrow($result))
			{
				return array('error' => true, 'error_msg' => '이미 등록된 주민등록번호입니다');
			}
			$db->sql_freeresult($result);
	}
