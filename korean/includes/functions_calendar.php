...
function get_event_topics(&$events, &$number, $start_date, $end_date, $limit=false, $start=0, $max_limit=-1, $fid='')
{
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		$short_title = str_cut($topic_title, $topic_title_length, '...'); 
		...
		{
			$nav_desc = '<a href="' . append_sid($phpbb_root_path . "./viewforum.$phpEx?" . POST_FORUM_URL . '=' . $row['forum_id']) . '" class="genmed">' . $row['forum_name'] . '</a>';
		}
		$template->assign_vars(array(
			...
			'L_DURATION'		=> $lang['Calendar_duration'],
		...
		$new_row['event_message']			= preg_replace($unhtml_specialchars_match, $unhtml_specialchars_replace, $row['post_text']);
...
function display_calendar($main_template, $nb_days=0, $start=0, $fid='')
{
	...
	else
	{
		...
		$start_date = mktime( 0,0,0, date('m', $cur_date), date('d', $cur_date), date('Y', $cur_date));
	...
	$template->assign_block_vars('_calendar_box', array(
		'L_CALENDAR'		=> '<a href="' . append_sid($phpbb_root_path . "./calendar.$phpEx?start=" . date( 'Ymd', time())) . '">'. $lang['Calendar'].'</a>' ,
	...
	for ($i=0; $i < $nb_rows; $i++)
	{
		...
		for ($j=0; $j < $nb_cells; $j++)
		{
			...
			$format = ( intval(date('Ymd', $offset_date)) == intval(date('Ymd', time())) ) ? '<font color=#f5989d><b>%s</b></font>' : '%s';
			$template->assign_block_vars('_calendar_box._row._cell', array(
				'WIDTH'		=> floor(100 / $nb_cells),
				'SPAN'		=> $span,
				'DATE'		=> sprintf( $format, date_dsp( $lang['CAL_DATE_FORMAT'].($full_month ? '' : ' [D]') , $offset_date)),
