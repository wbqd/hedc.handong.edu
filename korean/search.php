...
$search_id = ( isset($HTTP_GET_VARS['search_id']) ) ? $HTTP_GET_VARS['search_id'] : $HTTP_POST_VARS['search_id'];
$search_id_orig = $search_id;
...
//( isset($HTTP_POST_VARS['search_where']) ) ? $HTTP_POST_VARS['search_where'] : $HTTP_GET_VARS['mark']
//$search_where =  ( isset($HTTP_POST_VARS['search_where']) ) ? $HTTP_POST_VARS['search_where'] : 'Root';
if(isset($HTTP_POST_VARS['search_where'])) {
	$search_where =  $HTTP_POST_VARS['search_where'];
}
else if(isset($HTTP_GET_VARS['search_where'])) {
	$search_where =  $HTTP_GET_VARS['search_where'];
}
else {
	$search_where =  'c1';
}
...
//
$multibyte_charset = 'iso-8859-1, utf-8, big5, shift_jis, euc-kr, gb2312';
...
else if ( $search_keywords != '' || $search_author != '' || $search_id )
{
	...
	//
	if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'oneFieldSearch' || $search_id == 'multiFieldSearch'  || $search_id == 'unanswered' || $search_keywords != '' || $search_author != '' )
	{
		if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'oneFieldSearch' || $search_id == 'multiFieldSearch'  || ( $search_author != '' && $search_keywords == '' )  )
		{
			...
			else if ( $search_id == 'oneFieldSearch' )
			{			
				$oneFieldCondition = ( isset($HTTP_POST_VARS['oneFieldCondition']) ) ? trim($HTTP_POST_VARS['oneFieldCondition']) : trim($HTTP_GET_VARS['oneFieldCondition']);
				$oneFieldKeyword = ( isset($HTTP_POST_VARS['oneFieldKeyword']) ) ? trim($HTTP_POST_VARS['oneFieldKeyword']) : trim($HTTP_GET_VARS['oneFieldKeyword']);
				$oneFieldKeywordBackup = $oneFieldKeyword;
				$oneFieldKeyword =  addslashes('%' . str_replace('*', '', $oneFieldKeyword) . '%');

				if($oneFieldCondition == 'all') {
					$sql = "SELECT post_id
						FROM " . POSTS_TEXT_TABLE . "
						WHERE post_subject LIKE '$oneFieldKeyword' OR post_text LIKE '$oneFieldKeyword' OR remark1 LIKE '$oneFieldKeyword' OR remark2 LIKE '$oneFieldKeyword' OR remark3 LIKE '$oneFieldKeyword' OR remark4 LIKE '$oneFieldKeyword' OR remark5 LIKE '$oneFieldKeyword' OR remark6 LIKE '$oneFieldKeyword' OR remark7 LIKE '$oneFieldKeyword' OR remark8 LIKE '$oneFieldKeyword' OR remark9 LIKE '$oneFieldKeyword' OR remark10 LIKE '$oneFieldKeyword' OR remark11 LIKE '$oneFieldKeyword' OR remark12 LIKE '$oneFieldKeyword' OR remark13 LIKE '$oneFieldKeyword' OR remark14 LIKE '$oneFieldKeyword' OR remark15 LIKE '$oneFieldKeyword'
					";
				}
				else {					
					$sql = "SELECT post_id
						FROM " . POSTS_TEXT_TABLE . "
						WHERE $oneFieldCondition LIKE '$oneFieldKeyword' 
					";
				}
				$sort_by = 0;
				$sort_dir = 'DESC';
			}
			else if ( $search_id == 'multiFieldSearch' )
			{			
				$multiTemp = '';
				for($mCount=1;$mCount<=17;$mCount++) {
					$multiFieldKeyword = ( isset($HTTP_POST_VARS['multiFieldKeyword'.$mCount]) ) ? trim($HTTP_POST_VARS['multiFieldKeyword'.$mCount]) : trim($HTTP_GET_VARS['multiFieldKeyword'.$mCount]);
					if(!empty($multiFieldKeyword)) {
						$multiFieldCondition = ( isset($HTTP_POST_VARS['multiFieldCondition'.$mCount]) ) ? trim($HTTP_POST_VARS['multiFieldCondition'.$mCount]) : trim($HTTP_GET_VARS['multiFieldCondition'.$mCount]);

						$multiFieldKeyword =  addslashes('%' . str_replace('*', '', $multiFieldKeyword) . '%');

						$multiTemp .= empty($multiTemp) ? ' WHERE ' : ' AND ';

						$multiTemp .= "$multiFieldCondition LIKE '$multiFieldKeyword'";
					}				
				}
				...
				switch ( $split_search[$i] )
				{
					...
					default:
						...
						else
						{
							...
							$search_msg_only = ( $search_fields ) ? "OR post_text LIKE '$match_word' OR remark1 LIKE '$match_word' OR remark2 LIKE '$match_word' OR remark3 LIKE '$match_word' OR remark4 LIKE '$match_word' OR remark5 LIKE '$match_word' OR remark6 LIKE '$match_word' OR remark7 LIKE '$match_word' OR remark8 LIKE '$match_word' OR remark9 LIKE '$match_word' OR remark10 LIKE '$match_word' OR remark11 LIKE '$match_word' OR remark12 LIKE '$match_word' OR remark13 LIKE '$match_word' OR remark14 LIKE '$match_word' OR remark15 LIKE '$match_word'" : '';
							$sql = "SELECT post_id
								FROM " . POSTS_TEXT_TABLE . "
...
//-- add		
		...
		for ($i=0; $i < count($keys['id']); $i++)
		{
			...
			{
				if ( !in_array($tree['id'][ $keys['idx'][$i]], $forbiddenForumIDs) || $userdata['user_level'] == ADMIN || $search_author != '') {				
...
//-- fin mod : categories hierarchy ----------------------------------------------------------------
	...
	if ( $search_results != '' )
	{
		...
		{
			$sql = "SELECT pt.post_text, pt.bbcode_uid, pt.post_subject, p.*, f.forum_id, f.forum_name, t.*, u.username, u.user_id, u.user_sig, u.user_sig_bbcode_uid, pt.remark1, pt.remark2, pt.remark3, pt.remark4, pt.remark5, pt.remark6, pt.remark7, pt.remark8, pt.remark9, pt.remark10, pt.remark11, pt.remark12, pt.remark13, pt.remark14, pt.remark15   
		...
		if(!empty($search_keywords)) {
			$keywordDisplay = $search_keywords;
		}
		else if(!empty($oneFieldKeyword)) {
			$keywordDisplay = $oneFieldKeywordBackup;
		}
		else {
			$keywordDisplay = ( isset($HTTP_POST_VARS['keywordDisplay']) ) ? $HTTP_POST_VARS['keywordDisplay'] : $HTTP_GET_VARS['keywordDisplay'];
		}

		if (!empty($keywordDisplay)) {
			$keyword_display = '<font color=red>['.$keywordDisplay.']</font> ';
		}
		else {
			$keyword_display = '';
		}

		$l_search_matches = ( $total_match_count == 1 ) ? $keyword_display.sprintf($lang['Found_search_match'], $total_match_count) : $keyword_display.sprintf($lang['Found_search_matches'], $total_match_count);
		...
		for($i = 0; $i < count($searchset); $i++)
		{
			...
			$post_date = create_date('Y/m/d', $searchset[$i]['post_time'], $board_config['board_timezone']);
...
//-- fin mod : calendar ----------------------------------------------------------------------------
				...
				if(strstr($searchset[$i]['remark1'], 'กา')) list($dump,$searchset[$i]['remark1']) = explode("กา",$searchset[$i]['remark1']);
				if(strstr($searchset[$i]['remark2'], 'กา')) list($dump,$searchset[$i]['remark2']) = explode("กา",$searchset[$i]['remark2']);
				if(strstr($searchset[$i]['remark3'], 'กา')) list($dump,$searchset[$i]['remark3']) = explode("กา",$searchset[$i]['remark3']);
				if(strstr($searchset[$i]['remark4'], 'กา')) list($dump,$searchset[$i]['remark4']) = explode("กา",$searchset[$i]['remark4']);
				if(strstr($searchset[$i]['remark5'], 'กา')) list($dump,$searchset[$i]['remark5']) = explode("กา",$searchset[$i]['remark5']);
				if(strstr($searchset[$i]['remark6'], 'กา')) list($dump,$searchset[$i]['remark6']) = explode("กา",$searchset[$i]['remark6']);
				if(strstr($searchset[$i]['remark7'], 'กา')) list($dump,$searchset[$i]['remark7']) = explode("กา",$searchset[$i]['remark7']);
				if(strstr($searchset[$i]['remark8'], 'กา')) list($dump,$searchset[$i]['remark8']) = explode("กา",$searchset[$i]['remark8']);
				if(strstr($searchset[$i]['remark9'], 'กา')) list($dump,$searchset[$i]['remark9']) = explode("กา",$searchset[$i]['remark9']);
				if(strstr($searchset[$i]['remark10'], 'กา')) list($dump,$searchset[$i]['remark10']) = explode("กา",$searchset[$i]['remark10']);
				if(strstr($searchset[$i]['remark11'], 'กา')) list($dump,$searchset[$i]['remark11']) = explode("กา",$searchset[$i]['remark11']);
				if(strstr($searchset[$i]['remark12'], 'กา')) list($dump,$searchset[$i]['remark12']) = explode("กา",$searchset[$i]['remark12']);
				if(strstr($searchset[$i]['remark13'], 'กา')) list($dump,$searchset[$i]['remark13']) = explode("กา",$searchset[$i]['remark13']);
				if(strstr($searchset[$i]['remark14'], 'กา')) list($dump,$searchset[$i]['remark14']) = explode("กา",$searchset[$i]['remark14']);
				if(strstr($searchset[$i]['remark15'], 'กา')) list($dump,$searchset[$i]['remark15']) = explode("กา",$searchset[$i]['remark15']);
...
//-- fin mod : categories hierarchy ----------------------------------------------------------------
					...
					'REMARK1' => $searchset[$i]['remark1'],
					'REMARK2' => $searchset[$i]['remark2'],
					'REMARK3' => $searchset[$i]['remark3'],
					'REMARK4' => $searchset[$i]['remark4'],
					'REMARK5' => $searchset[$i]['remark5'],
					'REMARK6' => $searchset[$i]['remark6'],
					'REMARK7' => $searchset[$i]['remark7'],
					'REMARK8' => $searchset[$i]['remark8'],
					'REMARK9' => $searchset[$i]['remark9'],
					'REMARK10' => $searchset[$i]['remark10'],
					'REMARK11' => $searchset[$i]['remark11'],
					'REMARK12' => $searchset[$i]['remark12'],
					'REMARK13' => $searchset[$i]['remark13'],
					'REMARK14' => $searchset[$i]['remark14'],
					'REMARK15' => $searchset[$i]['remark15'],
...
//-- fin mod : categories hierarchy ----------------------------------------------------------------
		...
		$base_url = "search.$phpEx?search_id=$search_id&keywordDisplay=$keywordDisplay";
...
// }
//-- add
$s_forums = get_tree_option('',false,'c1');
//-- fin mod : categories hierarchy ----------------------------------------------------------------
