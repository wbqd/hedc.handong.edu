...
else
{
	while( $row = $db->sql_fetchrow($result) )
	{
		...
		{
			if($portal_name == 'exceptional_forums'){
				if(is_array($new['exceptional_forums'])) {
					$new['exceptional_forums'] = array_values($new['exceptional_forums']);
					$new['exceptional_forums'] = implode(',',$new['exceptional_forums']);
				}
				else {
					$new['exceptional_forums'] = '';
				}
			}
...
$exceptional_list = explode(',',$new['exceptional_forums']);

$exceptional_forums_list = '<select name="exceptional_forums[]" size="10" multiple="multiple">';

for($i=0;$i<count($tree['type']);$i++) {
	if($tree['type'][$i] == POST_FORUM_URL) {
		$selected = (in_array($tree['data'][$i]['forum_id'], $exceptional_list)) ? 'selected="selected"' : '';
		$exceptional_forums_list .= '<option value="'.$tree['data'][$i]['forum_id'].'" '. $selected.'>'.$tree['data'][$i]['forum_name'].'</option>';
	}
}

$exceptional_forums_list .= '</select>';
...
$template->assign_vars(array(
	...
	"EXCEPTIONAL_FORUMS_LIST" =>$exceptional_forums_list,
