 ...
 *
 *	$Id: admin_attach_cp.php,v 1.18 2003/08/30 15:47:39 acydburn Exp $
 *
 ***************************************************************************/
...
if ($submit_change && $view == 'attachments')
{
	...
	while ( $attachrow = $db->sql_fetchrow($result) )
	{
		...
		{			
			$url = $upload_dir . '/' . $attachrow['physical_filename'];
			$realsize = filesize($url);
			
			if ( ($attachrow['comment'] != $attachments['_' . $attachrow['attach_id']]['comment']) || (intval($attachrow['download_count']) != intval($attachments['_' . $attachrow['attach_id']]['download_count'])) || ($realsize!=intval($attachrow['filesize'])))
			{
				$sql = "UPDATE " . ATTACHMENTS_DESC_TABLE . " 
				SET comment = '" . $attachments['_' . $attachrow['attach_id']]['comment'] . "', download_count = " . intval($attachments['_' . $attachrow['attach_id']]['download_count']) . ", filesize = ". $realsize ." 
		...
		for ($i = 0; $i < count($attachments); $i++)
		{
			...
			for ($j = 0; $j < $num_ids; $j++)
			{
				if ($ids[$j]['post_id'] != 0)
				{
					...
					$post_titles[] = '<a href="' . $view_topic . '" class="genmed" target="_blank">' . $post_title . '</a>';
				}
				else
				{
...
{
	$pagination = generate_pagination('admin_attach_cp.' . $phpEx . '?view=' . $view . '&amp;mode=' . $mode . '&amp;order=' . $sort_order . '&amp;uid=' . $uid, $total_rows, $board_config['topics_per_page'], $start, FALSE).'&nbsp;';
