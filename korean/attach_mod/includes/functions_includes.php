 ...
 *
 *   $Id: functions_includes.php,v 1.32 2003/08/30 15:17:35 acydburn Exp $
 *
 *
 ***************************************************************************/
...
function attach_faq_include($lang_file)
{
	...
	if ($lang_file == 'lang_faq')
	{
		...
		{
			include($phpbb_root_path . 'language/lang_korean/lang_faq_attach.'.$phpEx);
		}
		else
		{
	...
	if ($admin_mode == 'user')
	{
		...
		if (!$submit && $mode != 'save')
		{
			...
			else
			{
				...
				$this_userdata = get_userdata($u_name, true);
				if( !$this_userdata )
				{
					message_die(GENERAL_MESSAGE, $lang['No_user_id_specified'] );
				}
...
function perform_attach_pageregister($session_page, $in_admin = FALSE)
{
	...
	switch ($session_page)
	{
		case (PAGE_UACP):
			...
			$location_url = ($in_admin) ? "index.$phpEx?pane=right" : "portal.$phpEx";
			break;
		case (PAGE_RULES):
