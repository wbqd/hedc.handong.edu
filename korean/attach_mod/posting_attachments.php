 ...
 *
 *   $Id: posting_attachments.php,v 1.60 2003/08/30 16:13:00 acydburn Exp $
 *
 *
 ***************************************************************************/
...
//		$priority = 'group;user';
		...
		{
			if (intval($attach_config[$default]) != 0 && intval($attach_config[$limit_type]) > intval($attach_config[$default]))
			{
				$attach_config[$limit_type] = $attach_config[$default];
			}
		}
			...
			else if ( ($edit_attachment) || ($update_attachment) || ($add_attachment) || ($preview) )
			{
				...
				if ( $update_attachment )
				{
					...
					if (!$error)
					{
						...
						$sql = "UPDATE " . ATTACHMENTS_DESC_TABLE . " 
						SET physical_filename = '" . $this->attach_filename . "', real_filename = '" . $this->filename . "', comment = '" . $comment . "', extension = '" . $this->extension . "', mimetype = '" . $this->type . "', filesize = " . $this->filesize . ", filetime = " . $this->filetime . ", thumbnail = " . $this->thumbnail . ", width = " . intval($width) . ", height = " . intval($height) . "
						WHERE attach_id = " . $attachment_id;
	...
	function do_insert_attachment($mode, $message_type, $message_id)
	{
		...
		if ($mode == 'attach_list')
		{
			...
			for ($i = 0; $i < count($this->attachment_list); $i++)
			{
				...
				else
				{
					...
					//
					$width = 0; $height = 0;
					if (substr($this->attachment_mimetype_list[$i], 0, 5) == 'image')
					{
						list($width, $height) = image_getdimension($upload_dir . '/' . $this->attachment_list[$i]);
					}
					$sql = "INSERT INTO " . ATTACHMENTS_DESC_TABLE . " (physical_filename, real_filename, comment, extension, mimetype, filesize, filetime, thumbnail, width, height) 
					VALUES ( '" . $this->attachment_list[$i] . "', '" . str_replace("'", "''", $this->attachment_filename_list[$i]) . "', '" . trim($this->attachment_comment_list[$i]) . "', '" . $this->attachment_extension_list[$i] . "', '" . $this->attachment_mimetype_list[$i] . "', " . $this->attachment_filesize_list[$i] . ", " . $this->attachment_filetime_list[$i] . ", " . $this->attachment_thumbnail_list[$i] . ", " . intval($width) . ", " . intval($height) . ")";
		...
		if ($mode == 'last_attachment')
		{
			if ( ($this->post_attach) && (!isset($HTTP_POST_VARS['update_attachment'])) )
			{
				...
				$sql = "INSERT INTO " . ATTACHMENTS_DESC_TABLE . " (physical_filename, real_filename, comment, extension, mimetype, filesize, filetime, thumbnail, width, height) 
				VALUES ( '" . $this->attach_filename . "', '" . str_replace("'", "''", stripslashes($this->filename)) . "', '" . trim($this->file_comment) . "', '" . $this->extension . "', '" . $this->type . "', " . $this->filesize . ", " . $this->filetime . ", " . $this->thumbnail . ", " . intval($width) . ", " . intval($height) . ")";
	...
	function display_attachment_bodies()
	{
		...
		if (($this->add_attachment_body) && ( count($this->attachment_list) < intval($attach_config['max_attachments'])))
		{
			...
			$template->assign_vars(array(
				...
				'L_ONLY_JPG' => $lang['only_jpg'], 
				'L_THUMB_EXTENSION' => $lang['thumb_extension'], 
