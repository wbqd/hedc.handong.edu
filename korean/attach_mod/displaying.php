 ...
 *
 *   $Id: displaying.php,v 1.46 2003/08/30 15:17:35 acydburn Exp $
 *
 *
 ***************************************************************************/
...
function init_display_template($template_var, $replacement, $filename = 'viewtopic_attach_body.tpl')
{
	...
	$complete_filename = $template->make_filename($filename);
	//die($complete_filename);
...
function topic_attachment_image($switch_attachment)
{
	...
	if ( (intval($switch_attachment) == 0) || (!( ($is_auth['auth_download']))) || (intval($attach_config['disable_mod'])) || ($attach_config['topic_icon'] == '') )
	{
		return ('');
	}
...
function display_post_attachments($post_id, $switch_attachment)
{
	...
	if ( (intval($switch_attachment) == 0) || (intval($attach_config['disable_mod'])) || (!( ($is_auth['auth_download']))) )
	{
		return;
	}
...
function display_assign_link($post_id)
{
	...
	if ( (intval($attach_config['disable_mod'])) || (!( ($is_auth['auth_download']))) )
	{
		return ('');
	}
...
function display_review_attachments($post_id, $switch_attachment, $is_auth)
{
	...
	if ( (intval($switch_attachment) == 0) || (intval($attach_config['disable_mod'])) || (!( ($is_auth['auth_download']))) || (intval($attach_config['attachment_topic_review']) == 0))
	{
		return;
	}
...
function init_display_review_attachments($is_auth)
{
	...
	if ( (intval($attach_config['disable_mod'])) || (!( ($is_auth['auth_download']))) || (intval($attach_config['attachment_topic_review']) == 0))
	{
		return;
	}
...
function display_attachments_preview($attachment_list, $attachment_filesize_list, $attachment_filename_list, $attachment_comment_list, $attachment_extension_list, $attachment_thumbnail_list)
{
	...
	if (count($attachment_list) != 0)
	{
		...
		for ($i = 0; $i < count($attachment_list); $i++)
		{
			...
			$comment = htmlspecialchars(stripslashes(trim(nl2br($attachment_comment_list[$i]))));
...
function display_attachments($post_id)
{
	...
	for ($i = 0; $i < $num_attachments; $i++)
	{
		...
		$comment = htmlspecialchars(stripslashes(trim(nl2br($attachments['_' . $post_id][$i]['comment']))));
		...
		if (!$denied)
		{
			...
			if ($thumbnail)
			{
				...
				$thumb_src_header = '<a href="#" onClick="window.open(\''.append_sid('download.' . $phpEx . '?id=' . $attachments['_' . $post_id][$i]['attach_id'])."','_blank','toolbar=no, location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=".($attachments['_' . $post_id][$i]['width']+20).",height=".($attachments['_' . $post_id][$i]['height']+30)."')\">";
				$thumb_src_tail = '</a>';
