 ...
 *
 *   $Id: privmsg.php,v 1.9 2003/08/30 15:05:45 acydburn Exp $
 *
 *
 ***************************************************************************/
...
if ( isset($HTTP_POST_VARS[POST_PM_URL]) || isset($HTTP_GET_VARS[POST_PM_URL]) )
{
	$privmsg_id = ( isset($HTTP_POST_VARS[POST_PM_URL]) ) ? intval($HTTP_POST_VARS[POST_PM_URL]) : intval($HTTP_GET_VARS[POST_PM_URL]);
}
else
{
	$privmsg_id = '';
...
if ( $mode == 'newpm' )
{
	...
	if ( $userdata['session_logged_in'] )
	{
		...
		$l_new_message_short = sprintf($lang['Click_view_privmsg'], '<a href="' . append_sid("privmsg.".$phpEx."?folder=inbox") . '" onclick="jump_to_inbox();return false;" target="_new">', '</a>');
	}
	else
	{
	...
	$template->assign_vars(array(
		...
		'L_MESSAGE_SHORT' => $l_new_message_short, 
...
{
	if ( !empty($HTTP_GET_VARS[POST_PM_URL]) )
	{
		$privmsgs_id = intval($HTTP_GET_VARS[POST_PM_URL]);
	}
	else
	{
	...
	if ( !$userdata['session_logged_in'] )
	{
		redirect(append_sid("login.$phpEx?redirect=privmsg.$phpEx&folder=$folder&mode=$mode&" . POST_PM_URL . "=$privmsgs_id", true));
	}
	...
	$post_urls = array(
		...
		'reply' => append_sid("privmsg.$phpEx?mode=reply&amp;" . POST_PM_URL . "=$privmsg_id"),
		'quote' => append_sid("privmsg.$phpEx?mode=quote&amp;" . POST_PM_URL . "=$privmsg_id"),
		'edit' => append_sid("privmsg.$phpEx?mode=edit&amp;" . POST_PM_URL . "=$privmsg_id")
	);
	$post_icons = array(
   ...
   $msn = $msn_img;
	...
	$private_message = unprepare_message($private_message);

	$u_from_user_profile = append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$user_id_from");
	$u_to_user_profile = append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$user_id_to");
	...
	$template->assign_vars(array(
		...
		'U_TO_USER_PROFILE' => $u_to_user_profile,
...
else if ( $submit || $refresh || $mode != '' )
{
	...
	if ( $submit && !$error )
	{
		...
		$msg = $lang['Message_sent'] . '<br /><br />' . sprintf($lang['Click_return_inbox'], '<a href="' . append_sid("privmsg.$phpEx?folder=inbox") . '">', '</a> ') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid("portal.$phpEx") . '">', '</a>');
	...
	else 
	{
		...
		else if ( $mode == 'reply' || $mode == 'quote' )
		{
			...
			if ( $mode == 'quote' )
			{
				...
				$msg_date = '<span title="'.create_date($board_config['default_dateformat'], $privmsg['privmsgs_date'], $board_config['board_timezone']).'">'.create_date("Y/m/d", $privmsg['privmsgs_date'], $board_config['board_timezone']).'</span>';
	...
	if ( $preview && !$error )
	{
		...
		if ( isset($privmsg_id) )
		{
			$s_hidden_fields .= '<input type="hidden" name="' . POST_PM_URL . '" value="' . $privmsg_id . '" />';
		}
		...
		$preview_message = unprepare_message($preview_message);
...
if ( $row = $db->sql_fetchrow($result) )
{
	do
	{
		...
		$u_subject = append_sid("privmsg.$phpEx?folder=$folder&amp;mode=read&amp;" . POST_PM_URL . "=$privmsg_id");

		//$msg_date = create_date($board_config['default_dateformat'], $row['privmsgs_date'], $board_config['board_timezone']);

		$msg_date = '<span title="'.create_date($board_config['default_dateformat'], $row['privmsgs_date'], $board_config['board_timezone']).'">'.create_date("Y/m/d", $row['privmsgs_date'], $board_config['board_timezone']).'</span>';
