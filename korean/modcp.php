 ...
 *
 *   $Id: modcp.php,v 1.10 2003/08/30 15:05:44 acydburn Exp $
 *
 ***************************************************************************/
	...
	case 'delete':
		...
		if ( $confirm )
		{
			...
			if ( $post_id_sql != '' )
			{
				...
				// Log actions MOD Start
				log_action('delete[posts]', 0, $userdata['user_id'], $userdata['username']);
				// Log actions MOD End
			...
			if ( $vote_id_sql != '' )
			{
				...
				// Log actions MOD Start
				log_action('delete[votes]', 0, $userdata['user_id'], $userdata['username']);
				// Log actions MOD End
			}
			...
			// Log actions MOD Start
			log_action('delete[watched-posts]', 0, $userdata['user_id'], $userdata['username']);
			// Log actions MOD End
		...
		else
		{
			...
			$template->assign_vars(array(
				...
				'U_BACK' => append_sid("viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id"),
...
//-- add
				...
				'S_FORUM_SELECT' => selectbox('new_forum', false, '1', '1'),
//-- fin mod : categories hierarchy ---------------------------------------------------------------- 
...
//-- fin mod : categories hierarchy ----------------------------------------------------------------
		...
		else
		{
			...
			$sql = "SELECT u.username, p.*, pt.post_text, pt.bbcode_uid, pt.post_subject, p.post_username, pt.remark1, pt.remark2, pt.remark3, pt.remark4, pt.remark5, pt.remark6, pt.remark7, pt.remark8, pt.remark9, pt.remark10, pt.remark11, pt.remark12, pt.remark13, pt.remark14, pt.remark15 
...
//-- add
					'S_FORUM_SELECT' => selectbox("new_forum_id", false, '1', '1'))
//-- fin mod : categories hierarchy ----------------------------------------------------------------
				);
				...
				for($i = 0; $i < $total_posts; $i++)
				{
					...
					$checkbox = ( $i > 0 ) ? '<input type="checkbox" class="formstyle3" name="post_id_list[]" value="' . $post_id . '" />' : '&nbsp;';

					if(strstr($postrow[$i]['remark1'], 'กา')) list($dump,$postrow[$i]['remark1']) = explode("กา",$postrow[$i]['remark1']);
					if(strstr($postrow[$i]['remark2'], 'กา')) list($dump,$postrow[$i]['remark2']) = explode("กา",$postrow[$i]['remark2']);
					if(strstr($postrow[$i]['remark3'], 'กา')) list($dump,$postrow[$i]['remark3']) = explode("กา",$postrow[$i]['remark3']);
					if(strstr($postrow[$i]['remark4'], 'กา')) list($dump,$postrow[$i]['remark4']) = explode("กา",$postrow[$i]['remark4']);
					if(strstr($postrow[$i]['remark5'], 'กา')) list($dump,$postrow[$i]['remark5']) = explode("กา",$postrow[$i]['remark5']);
					if(strstr($postrow[$i]['remark6'], 'กา')) list($dump,$postrow[$i]['remark6']) = explode("กา",$postrow[$i]['remark6']);
					if(strstr($postrow[$i]['remark7'], 'กา')) list($dump,$postrow[$i]['remark7']) = explode("กา",$postrow[$i]['remark7']);
					if(strstr($postrow[$i]['remark8'], 'กา')) list($dump,$postrow[$i]['remark8']) = explode("กา",$postrow[$i]['remark8']);
					if(strstr($postrow[$i]['remark9'], 'กา')) list($dump,$postrow[$i]['remark9']) = explode("กา",$postrow[$i]['remark9']);
					if(strstr($postrow[$i]['remark10'], 'กา')) list($dump,$postrow[$i]['remark10']) = explode("กา",$postrow[$i]['remark10']);
					if(strstr($postrow[$i]['remark11'], 'กา')) list($dump,$postrow[$i]['remark11']) = explode("กา",$postrow[$i]['remark11']);
					if(strstr($postrow[$i]['remark12'], 'กา')) list($dump,$postrow[$i]['remark12']) = explode("กา",$postrow[$i]['remark12']);
					if(strstr($postrow[$i]['remark13'], 'กา')) list($dump,$postrow[$i]['remark13']) = explode("กา",$postrow[$i]['remark13']);
					if(strstr($postrow[$i]['remark14'], 'กา')) list($dump,$postrow[$i]['remark14']) = explode("กา",$postrow[$i]['remark14']);
					if(strstr($postrow[$i]['remark15'], 'กา')) list($dump,$postrow[$i]['remark15']) = explode("กา",$postrow[$i]['remark15']);
	...
	default:
		...
		//	make_jumpbox('modcp.'.$phpEx, $forum_id);
		//}
