 ...
 *
 *   $Id: uacp.php,v 1.12 2003/08/30 14:02:32 acydburn Exp $
 *
 *
 ***************************************************************************/
...
if (count($attachments) > 0)
{
	for ($i = 0; $i < count($attachments); $i++)
	{
		...
		{
			$delete_box = '<input type="checkbox" name="delete_id_list[]" value="' . $attachments[$i]['attach_id'] . '"  class="formstyle3" />';

			for ($j = 0; $j < count($delete_id_list); $j++)
			{
				if ($delete_id_list[$j] == $attachments[$i]['attach_id'])
				{
					$delete_box = '<input type="checkbox" name="delete_id_list[]" value="' . $attachments[$i]['attach_id'] . '" checked  class="formstyle3" />';
					break;
				}
			}
