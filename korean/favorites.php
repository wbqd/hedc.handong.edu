...
{ 
   ...
   { 
      ...
      { 
		...
		log_action('delete[favorite]', $topic_id, $userdata['user_id'], $userdata['username']);
		// Log actions MOD End
      } 
      if ( !(isset($HTTP_GET_VARS['t'])) ) 
      { 
...
$sql = "SELECT " . $table_prefix . "favorites.topic_id as old_id, " . $table_prefix . "topics.*
...
if(!$row['topic_id'])
{
	$topic_title = $lang['deleted_post'];
	$u_remove = append_sid("favorites.$phpEx?mode=remove&t=" . $row['old_id']);	
}
else {
	$topic_title = '<a href="'.append_sid("viewtopic.$phpEx?t=" . $row['topic_id']).'">'.$row['topic_title'].'</a>';
...
//END // 1.0.2 update // Simple Post Icon
   $template->assign_block_vars('topicrow', array(
	...
	'L_TOPIC_TITLE' => $topic_title,
	'LAST_POST_TIME' => '<span title="'.create_date($board_config['default_dateformat'], $row['topic_time'], $board_config['board_timezone']).'">'.create_date("Y/m/d", $row['topic_time'], $board_config['board_timezone']).'</span>',
