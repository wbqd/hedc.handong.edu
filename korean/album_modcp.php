 ...
 *
 *   $Id: album_modcp.php,v 2.0.5 2003/03/02 20:07:50 ngoctu Exp $
 *
 *
 ***************************************************************************/
...
else
{
	...
	else if ($mode == 'delete')
	{
		...
		else
		{
			...
			{
				if( ($filerow['pic_thumbnail'] != '') and (@file_exists(ALBUM_CACHE_PATH . $filerow['pic_thumbnail'])) )
				{
					@unlink(ALBUM_CACHE_PATH . $filerow['pic_thumbnail']);
				}
			...
			log_action('delete[album-pics]', 0, $userdata['user_id'], $userdata['username']);
			// Log actions MOD End
