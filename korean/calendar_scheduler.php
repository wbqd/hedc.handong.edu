...
$year_1 = substr($year,0,1);
$year_2 = substr($year,1,1);
$year_3 = substr($year,2,1);
$year_4 = substr($year,3,1);
...
for ($j=0; $j < $nb_rows; $j++)
{
	...
	for ($i=0; $i <= 6; $i++)
	{
		...
		if ($days[$cur] > 0)
		{
			...
			$cur = sprintf('<a href="%s" class="gen"><b>%s</b></a><br>', $url, $cur);
			for ($l=0; $l < $temp_day; $l++) {
				$cur .= "<img src='templates/".$theme['template_name']."/images/calendar/note-icon.gif' border=0>";
				if((($l+1)%4) == 0) $cur .= "<br>";
			}
		}
...
for ($i = 0; $i < count($events); $i++)
{
	...
	$row['post_text']				= $events[$i]['event_message'];
...
$footer = '';
topic_list('TOPIC_LIST_SCHEDULER', 'calendar_list_box', $topic_rowset, $title, $split_type, $display_nav_tree, $footer );
